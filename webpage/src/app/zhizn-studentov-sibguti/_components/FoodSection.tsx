        ...
        <section id="food" className="py-20">
            <div className="mx-auto w-full max-w-[1400px] px-4 md:px-8">
                <div className="relative overflow-hidden rounded-[40px] bg-[var(--background)] p-6 md:p-12 lg:p-16 border-4 border-[var(--foreground)] shadow-[12px_12px_0px_0px_var(--foreground)]">
                    ...
                    <div className="relative z-10 mb-12 flex flex-col lg:flex-row lg:items-end justify-between gap-6 border-b-4 border-[var(--foreground)] pb-8">
                        <div>
                            ...
                            <h2 className="text-4xl sm:text-6xl lg:text-8xl font-black tracking-tighter leading-none uppercase text-[var(--foreground)] whitespace-nowrap overflow-hidden text-ellipsis">
                    ...
                    <div className="grid grid-cols-1 gap-10 lg:grid-cols-12">
                        <div className="lg:col-span-5 flex flex-col gap-6 order-2 lg:order-1">
                            ...
                            {locations.map((loc, i) => (
                                ...
                                >
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="text-xs font-black text-[var(--accent)] uppercase tracking-wider">{loc.address}</div>
                            ...
                            <div className="flex items-center gap-4 p-4 border-4 border-[var(--foreground)] bg-[var(--accent)] shadow-[8px_8px_0px_0px_var(--foreground)] cursor-default">
                                ...
                                <div className="flex-1">
                                    <h4 className="font-black text-lg uppercase text-[var(--background)] leading-none mb-1">Кофемашины в каждом корпусе!</h4>
                                    <div className="text-xs font-black text-[var(--background)] uppercase tracking-wider opacity-90">Ну кто же откажется от свежего кофе с утра?!</div>
                                </div>
                            </div>

                            <button
                                onClick={() => setShowTable(!showTable)}
                                className="group relative mt-auto flex items-center justify-center gap-3 w-full border-4 border-[var(--foreground)] bg-[var(--foreground)] py-6 text-sm font-black uppercase tracking-[0.2em] text-[var(--background)] transition-all hover:bg-[var(--accent)] hover:text-white cursor-pointer overflow-hidden shadow-[8px_8px_0px_0px_var(--accent)] hover:translate-x-1 hover:translate-y-1 hover:shadow-none"
                            >
                        ...
                        <div className="lg:col-span-7 relative h-[400px] md:h-[500px] border-4 border-[var(--foreground)] shadow-[16px_16px_0px_0px_var(--foreground)] overflow-hidden bg-[var(--foreground)] group order-1 lg:order-2">
                            <AnimatePresence initial={false} custom={currentIndex > 0 ? 1 : -1} mode="wait">
                                ...
                                >
                                    ...
                                    />
                                    <div className="absolute inset-0 bg-gradient-to-t from-[var(--foreground)]/90 via-[var(--foreground)]/20 to-transparent" />
                                    ...
                                    <div className="absolute bottom-6 left-6 right-24">
                                        <div className="bg-[var(--background)] border-4 border-[var(--foreground)] p-6 shadow-[8px_8px_0px_0px_var(--accent)] -rotate-1">
                                            <h3 className="text-2xl md:text-3xl font-black text-[var(--foreground)] leading-none uppercase tracking-tighter mb-2">
                            ...
                            <div className="absolute right-4 bottom-4 flex flex-col gap-2 z-20">
                                <button
                                    ...
                                    className="h-14 w-14 flex items-center justify-center bg-[var(--background)] border-4 border-[var(--foreground)] shadow-[4px_4px_0px_0px_var(--foreground)] transition-all hover:translate-x-1 hover:translate-y-1 hover:shadow-none cursor-pointer active:scale-95"
                                >
                                    <LuChevronLeft size={32} className="text-[var(--foreground)]" />
                                </button>
                                <button
                                    onClick={() => handleManualChange('next')}
                                    className="h-14 w-14 flex items-center justify-center bg-[var(--accent)] border-4 border-[var(--foreground)] shadow-[4px_4px_0px_0px_var(--foreground)] transition-all hover:translate-x-1 hover:translate-y-1 hover:shadow-none cursor-pointer active:scale-95"
                                >
                        ...
                        {showTable && (
                            ...
                            >
                                <div className="mt-12 border-4 border-[var(--foreground)] bg-[var(--background)] shadow-[12px_12px_0px_0px_var(--foreground)]">
                                    <div className="bg-[var(--foreground)] px-6 py-4 flex items-center justify-between">
                                        <div className="flex items-center gap-4 text-[var(--background)]">
                                            <LuCoffee size={24} className="text-[var(--accent)]" />
                                            <div>
                                                <h3 className="text-xl font-black uppercase tracking-tighter">Реестр точек питания и сопутствующих объектов</h3>
                                    ...
                                    <div className="overflow-x-auto">
                                        <table className="w-full text-left border-collapse">
                                            ...
                                            <tr className="bg-[var(--background)] border-b-4 border-[var(--foreground)] text-[var(--foreground)]">
                                                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest border-r-2 border-[var(--foreground)]">Тип</th>
                                                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest border-r-2 border-[var(--foreground)]">Адрес</th>
                                                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest border-r-2 border-[var(--foreground)] text-center">Площадь (м²)</th>
                                                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest border-r-2 border-[var(--foreground)] text-center">Места</th>
                                                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest">Доступность</th>
                                            </tr>
                                            </thead>
                                            ...
                                            {fullData.map((row, i) => (
                                                <tr key={i} className="hover:bg-[var(--accent)]/10 transition-colors">
                                                    <td className="px-6 py-4 text-sm font-black border-r-2 border-[var(--foreground)]">{row.type}</td>
                                                    <td className="px-6 py-4 text-xs opacity-80 border-r-2 border-[var(--foreground)]">{row.address}</td>
                                                    <td className="px-6 py-4 text-sm font-black text-center border-r-2 border-[var(--foreground)]">{row.area}</td>
                                                    <td className="px-6 py-4 text-lg font-black text-[var(--accent)] text-center border-r-2 border-[var(--foreground)]">{row.seats}</td>
                                                    <td className="px-6 py-4">
                                                        <div className={`inline-flex items-center justify-center px-3 py-1 text-[10px] font-black border-2 border-[var(--foreground)] ${row.accessibility === 'ОВЗ' ? 'bg-[#4ade80] text-[var(--foreground)] shadow-[2px_2px_0px_0px_var(--foreground)]' : 'bg-[var(--background)] text-[var(--foreground)] opacity-70'}`}>
                                                            {row.accessibility}
                                                        </div>
                                                    </td>
                                                </tr>
